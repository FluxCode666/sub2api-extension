...
- 优先使用现有 token、Tailwind utility、shadcn 组件和 `lucide-react` 图标；工具按钮使用图标和 tooltip，避免用长文本模拟图标。
...
4. 实现时复用现有 token、组件、Store、middleware、命令和测试夹具；不要为了满足 skill 示例而引入新的框架、颜色体系或目录层级。
