...
成本配置的 `GET /api/aux/admin/ops/cost-config` 位于 `AdminGuard` 下，继续使用 `X-Aux-Session`。配置了 Sub2API 数据库时，每次打开或刷新页面先从 `accounts` 读取完整账号列表（包括供历史检索的软删除账号），同步账号元数据和倍率至附属库，再返回原有 `global/accounts/last_sync_at` 响应；账号出现不依赖 `usage_logs`，无需先发生调用或点击“立即同步倍率”。OAuth 采购价、API 手工倍率和合并计费组仍由附属库保存，刷新不覆盖这些设置。列表默认隐藏已删除账号，名称/ID 搜索、类型/平台/日期筛选与分页仍由前端处理。

未配置 Sub2API 数据库时，成本配置仍可读取已有本地数据；配置了账号来源后，读取或同步失败会返回标准错误 envelope（已识别的数据库不可用为 503，其他查询/存储错误为 500），不会用空账号列表掩盖失败。手工、定时与页面触发的同步在同一服务实例内串行执行，等待可随请求取消；保存合并计费组后直接读取本地结果，避免上游故障把已完成的保存报成失败。本次不修改数据库结构、响应字段或环境变量，无需额外迁移；回退旧版本可继续读取已保存配置。
