...
合并计费弹窗继续使用完整账号列表，但账号下拉按每页 10 个匹配选项加载，滚动到底部自动追加下一页，避免大量账号同时注册到选择组件导致页面卡顿。搜索覆盖名称、ID、平台、类型和计费组，沿用 cmdk 的模糊匹配与相关性排序；先搜索全部账号，再按页展示。切换搜索或关闭下拉不会丢失已选账号，关闭合并弹窗才清空选择。可选项的 `data-disabled="false"` 不应用禁用光标，只有 `data-disabled="true"` 匹配禁用样式。该下拉使用 Radix 模态 Popover，避免外层 Dialog 的滚动锁拦截选项滚轮；按 Esc 关闭后焦点回到账号选择按钮。
