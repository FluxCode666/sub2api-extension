...
运营中心路径为 `/admin/ops/consumption` 与 `/admin/ops/cost-config`。消费核算趋势图按天展示 API 账号请求量、Token 和 API 成本，支持悬停查看每日 API 成本详情并显示 X/Y 轴刻度；每日明细通过 API / OAuth 账号类型 tab 切换，按日期展示每日汇总，不按账号拆行。OAuth Tab 仅展示当天利润（收入），另有独立的 OAuth 回本分析板块展示账号创建时间、过期时间（括号标注剩余天数，空值显示“未获取到”）、请求数、账号级收入、采购成本、利润、回本进度、待回本金额和状态；OAuth 利润按 `OAuth 收入 − OAuth 采购成本 − OAuth 收入 × 税点` 计算，不混入合并计费组中的 API 收入与成本。每日明细不展示无法可靠核算的 OAuth 成本和回本进度列。API Tab 展示收入可用时的毛利、税额、税后利润和利润率，没有明确收费字段时这些利润列以不可用占位显示，避免把 provider 实际成本冒充收入。OAuth 回本分析和账号成本明细默认按账号创建时间倒序排列，筛选后、分页前均可按创建时间、收入、利润或请求数进行独立的单列正序/倒序排序，切换排序会回到第一页。账号成本明细支持账号名或计费组搜索、账号创建时间范围、账号类型和平台筛选，并展示收入、毛利、税额和利润；页面不再展示倍率/核算口径列，没有明确收费字段时收入与利润相关列以不可用占位显示。Sub2API `usage_logs.actual_cost` 是用户实际扣费收入；兼容部署可使用明确表示用户收费的 `charged_amount`、`billed_amount`、`user_charge` 或 `request_amount`，但 `total_cost`、`account_stats_cost`、`provider_cost` 和 `upstream_cost` 只能作为成本。OAuth 账号采购成本按账号一次性计入区间总账，不归集到某个使用日。日期范围最多 93 天；全局默认配置、税点和每个账号的独立成本配置写入扩展自有数据库，不会修改 Sub2API 数据库。明确收费字段可用时，税点以百分比配置，例如 `6` 表示 `6%`，区间总账按 `税前利润 = 总收入 − API 成本 − OAuth 采购成本`、`税后利润 = 税前利润 − 总收入 × 税点` 计算。OAuth 账号按账号 ID 配置采购单价，并在筛选范围内按独立账号计入；同一计费组中的 OAuth 记录只计一次采购成本，组内只需一个账号明确设置采购单价，其他未设置账号继承该组成本（即使设置成本的成员在查询区间内没有调用）；若多个账号都明确设置，金额必须一致，明确填写的 `0` 也算已设置。全组未设置时使用全局默认值，不会将继承值回填到账号配置。API 账号按账号 ID 配置倍率，优先级为手工覆盖、已同步倍率、Sub2API 当前 `accounts.rate_multiplier`，都不存在时才使用全局默认倍率；同一计费组中的多个 API 账号分别按各自有效倍率计算后汇总。API 与 OAuth 账号也可以加入同一计费组，系统会分别套用 API 倍率和 OAuth 采购成本规则后汇总；OAuth 账号的 `account_rate_multiplier` 快照不会被误算为 API 成本。历史 API usage log 优先使用 `account_rate_multiplier` 快照，因此上游倍率后续变化不会改写已经发生的成本。

成本配置的 `GET /api/aux/admin/ops/cost-config` 位于 `AdminGuard` 下，继续使用 `X-Aux-Session`。配置了 Sub2API 数据库时，每次打开或刷新页面先从 `accounts` 读取完整账号列表（包括供历史检索的软删除账号），只读合并附属库已保存的账号成本配置，再返回原有 `global/accounts/last_sync_at` 响应；账号出现不依赖 `usage_logs`，无需先发生调用或点击“立即同步倍率”。OAuth 采购价、API 手工倍率和合并计费组仍由附属库保存，刷新不覆盖这些设置。列表默认隐藏已删除账号，名称/ID 搜索、类型/平台/日期筛选与分页仍由前端处理。

合并计费弹窗继续使用完整账号列表，但账号下拉按每页 10 个匹配选项加载，滚动到底部自动追加下一页，避免大量账号同时注册到选择组件导致页面卡顿。搜索覆盖名称、ID、平台、类型和计费组，沿用 cmdk 的模糊匹配与相关性排序；先搜索全部账号，再按页展示。切换搜索或关闭下拉不会丢失已选账号，关闭合并弹窗才清空选择。可选项的 `data-disabled="false"` 不应用禁用光标，只有 `data-disabled="true"` 匹配禁用样式。该下拉使用 Radix 模态 Popover，避免外层 Dialog 的滚动锁拦截选项滚轮；按 Esc 关闭后焦点回到账号选择按钮。单账号保存和合并计费失败使用与成功反馈相同的右上角 Sonner 通知，以 error 类型显示错误，不再要求滚动到页面顶部查看；失败后保留编辑和选择以便重试。

未配置 Sub2API 数据库时，成本配置仍可读取已有本地数据；配置了账号来源后，读取或同步失败会返回标准错误 envelope（已识别的数据库不可用为 503，其他查询/存储错误为 500），不会用空账号列表掩盖失败。页面读取不执行逐账号写库，也不等待全量同步锁。手工与定时同步仍负责元数据持久化，并在同一服务实例内串行执行，等待可随请求取消；保存独立成本或合并计费组时只同步所选账号，首次建档与后台同步冲突时重读记录并保留已有成本设置。合并计费保存后复用写入前的上游快照，未建档账号仍会显示，也不再次依赖上游读取。前端默认请求超时仍为 15 秒，超时显示“请求超时，请稍后重试”；首次加载失败仅展示错误与重新加载入口，不展示默认费用或零账号。本次不修改数据库结构、响应字段或环境变量，无需额外迁移；回退旧版本可继续读取已保存配置。

OAuth 回本分析的过期时间优先采用管理员填写的 `accounts.expires_at`；未填写时只读提取 `accounts.credentials` 中的 `subscription_expires_at`，影子账号缺少有效订阅日期时再通过 `parent_account_id` 读取母账号的同名字段。只提取日期，不加载或返回完整凭据，绝不将可刷新的 `credentials.expires_at`（token 到期）用于回本分析。订阅日期按带时区的 RFC3339 解析；缺少字段、空值、非法值或零时间视为未知，显示“未获取到”，不代表永久有效。API 账号不使用此订阅回退规则。上游订阅未获取成功时，附属系统也无法推断，需先在 Sub2API 更新账号订阅信息。

这些日期沿用已有同步机制：启动时尝试同步，默认每 300 秒更新，也可在成本配置页点击“立即同步倍率”；消费核算读取最近成功持久化的日期，订阅续期或清空后需再次同步。合并计费组沿用最早的已知期限。无需新增迁移、接口字段或环境变量，升级后同步账号并刷新消费核算；回退后再次同步将恢复只读取手填期限的行为。

回归验证应覆盖：无 `usage_logs` 表时首次加载账号、添加未使用账号后再次加载、配置采购价并合并计费后刷新仍保留设置、上游软删除与恢复、账号来源失败和并发同步取消。
...
- [ ] Sub2API 新增尚无使用记录的账号后，打开或刷新「成本配置」即可展示，并能保存 OAuth 单号成本/API 手工倍率或参与合并计费。
...
ToB 顶部导航以管理员保存的 `navigationItems` 列表为准。删除「全球网络」、修改其链接或显式保存空数组后，保存响应和后续读取都不会自动补回该菜单；旧配置缺少 `navigationItems` 或值为 `null` 时仍使用 ToB 默认导航。删除导航项仅移除菜单入口，全球网络地图继续展示。
