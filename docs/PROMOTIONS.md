...
- 单个用户福利金额上限：按“用户 + 活动”累计已发放返利金额限制该用户可领取的总额；填写 `0` 表示不限。例如配置 `200` 时，该用户在该活动累计领取返利达到 200 元后不能继续领取。若本次选择的订单返利超过剩余额度，则只发放剩余额度；选择多个订单时按返利金额从小到大优先发放。
- 订单的支付时间必须落在活动开始时间（含）至结束时间（不含）的范围内；未设置某一端时间时，该端不设限制。

用户端 `/promotions` 通过 Sub2API `custom_menu_items` 以 iframe 打开，侧边栏展示进行中、未来 3 天内开始和已结束的已上架活动；每个活动在侧边栏和详情区域都展示活动时间范围。即将开始的活动按开始时间升序排列，用户可以提前查看活动说明和返利规则，但活动开始前不会加载订单，也不能领取返利。已结束活动默认折叠，并按结束时间倒序排列。所有订单请求经过 `UserGuard`，订单归属由后端从 Sub2API 数据库重新校验。一个充值订单在所有促销活动中只能领取一次，领取记录使用 `payment_order_id` 全局唯一索引；重复领取返回 409。
...
当用户在某活动的累计返利已达到上限时，订单接口和领取接口会拒绝继续领取；用户端提示“返利金额达到上限”。如果本次选择的订单返利超过剩余额度，接口会按规则发放剩余金额，未获得返利的订单保持未领取状态。
