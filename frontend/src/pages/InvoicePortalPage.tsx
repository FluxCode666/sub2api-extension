...
export default function InvoicePortalPage() {
  ...
  const refreshOrders = useCallback(async () => {
    ...
    try {
      ...
      setOrders(refreshed)
      setSelected((current) => current !== null && refreshed.some((order) => order.payment_order_id === current) ? current : null)
    } catch (caught) {
  ...
  const selectOrder = (id: number) => { setSubmitCompleted(false); setSelected(id) }
  ...
  const submit = async (event: FormEvent) => {
    ...
    if (selected === null) { setError('请先选择一笔可开票的充值订单。'); return }
    setSubmitting(true)
    try {
      await apiClient.post<AuxEnvelope<InvoiceRequest>>('/invoices/requests', { ...form, order_ids: [selected] })
      setSelected(null); setForm(emptyForm); setSubmitCompleted(true); setNotice('发票申请已提交。管理员开具并上传后，会在申请记录中出现下载入口。')
    ...
    <div ref={portalRef} className="invoice-portal min-h-screen overflow-x-hidden">
      ...
      <main id="invoice-main">
        <section className="invoice-hero">
          ...
          <div className="invoice-hero-content invoice-shell"><div className="invoice-hero-copy"><p className="invoice-eyebrow"><span className="invoice-eyebrow-dot" />充值完成，开票资料随时可用</p><h1>充值，按实付金额开票。</h1><p className="invoice-hero-lede">选择一笔已完成的余额充值订单，确认企业抬头与收票信息，提交后由管理员人工开具并上传发票。</p><div className="invoice-hero-actions"><a href="#invoice-orders" className="invoice-button invoice-button--light">选择充值订单 <ArrowDownRight className="h-4 w-4" /></a><a href="#invoice-profile" className="invoice-button invoice-button--ghost">查看默认资料 <ArrowRight className="h-4 w-4" /></a></div></div><div className="invoice-hero-footnote"><ShieldCheck className="h-4 w-4" />仅展示当前登录账号的充值记录</div></div>
        </section>
