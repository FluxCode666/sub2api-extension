...
// 管理端从上级目录 pages/ 加载组件
type DynamicComponentProps = { metadata?: Record<string, unknown>; pageId?: string }
...
export default function AdminDynamicPage() {
  ...
  useEffect(() => {
    ...
    apiClient
      ...
      .catch((error: unknown) => {
        console.error('[AdminDynamicPage] failed to load page', { slug, error })
        if (cancelled) return
    ...
    if (page?.content_type === 'react' && page.content_react) {
      ...
      if (isCodeMode) {
        ...
        compileAndCreateComponent(reactContent)
          ...
          .catch((err) => {
            console.error('[AdminDynamicPage] failed to compile React page', err)
        ...
        if (loader) {
          loader()
            ...
            .catch((err) => {
              console.error('[AdminDynamicPage] failed to load React module', { componentPath, error: err })
  ...
  if (state === 'loading') {
    return (
      <div className="aux-admin-page-host aux-admin-page-host--loading">
  ...
  return (
    <div className={fullBleed ? 'aux-admin-page-host aux-admin-page-host--full-bleed' : 'aux-admin-page-host'}>
      {!fullBleed && (
        <header className="aux-page-header">
          <span className="aux-page-kicker">管理内容</span>
          <h1>{page.title}</h1>
        </header>
      )}
      <SandboxRenderer
