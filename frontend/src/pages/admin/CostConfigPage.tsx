  ...
  return (
    <div className="aux-cost-page aux-cost-config-page">
      ...
      <section className="aux-cost-panel aux-account-config-panel">
        ...
        <p id="account-date-hint" className="aux-account-filter-hint">账号从账号列表自动获取，无需使用记录。默认仅显示未删除账号；搜索账号名或 ID 可查询已删除账号。按创建时间倒序排列，日期范围包含本地结束当天，未知创建时间排在最后且不参与日期筛选。</p>
        <div className="aux-account-table-scroll"><Table className="aux-account-table"><TableHeader><TableRow><TableHead className="aux-account-name-column">账号</TableHead><TableHead className="aux-account-created-column" aria-sort="descending">创建时间 ↓</TableHead><TableHead className="aux-account-type-column">类型 / 平台</TableHead><TableHead className="aux-account-oauth-cost-column">OAuth 单号成本</TableHead><TableHead className="aux-account-api-multiplier-column">API 成本倍率</TableHead><TableHead className="aux-account-sync-column">同步倍率</TableHead><TableHead className="aux-account-group-column">合并计费组</TableHead><TableHead className="aux-account-actions-column">操作</TableHead></TableRow></TableHeader><TableBody>
          {visibleAccounts.length === 0 ? <TableRow><TableCell colSpan={8} className="aux-cost-empty-cell">{hasFilters ? "没有符合筛选条件的账号，请调整或重置筛选。" : draftAccounts.length ? "暂无未删除账号，输入账号名或 ID 可查询已删除账号。" : "暂无账号，请先在 Sub2API 添加账号后刷新页面。"}</TableCell></TableRow> : visibleAccounts.map((account) => <TableRow key={account.account_id}>
            ...
            <TableCell className="aux-account-created-column aux-account-created-at">{account.account_created_at ? formatAccountCreatedAt(account.account_created_at) : "—"}</TableCell>
            <TableCell className="aux-account-type-column"><div className="aux-account-type-stack"><span className={`aux-account-type aux-account-type--${account.account_type}`}>{account.account_type === "oauth" ? "OAuth" : "API"}</span><small>{account.platform || "—"}</small></div></TableCell>
            <TableCell className="aux-account-oauth-cost-column">{account.account_type === "oauth" ? <Input aria-label={`账号 ${account.account_id} 的 OAuth 单号成本`} className="aux-account-number" type="number" min="0" step="0.01" value={account.oauth_account_cost ?? ""} placeholder="未配置" onChange={(event) => updateAccount(setDraftAccounts, account.account_id, { oauth_account_cost: event.target.value === "" ? null : Number(event.target.value) })} /> : <span className="aux-account-muted">不适用</span>}</TableCell>
            ...
            <TableCell className="aux-account-sync-column">{account.account_type === "api" ? <><strong>{account.synced_api_multiplier?.toFixed(4) ?? "—"}</strong><small>{account.last_synced_at ? formatSyncTime(account.last_synced_at) : "未同步"}</small></> : <span className="aux-account-muted">采购价独立配置</span>}</TableCell>
            <TableCell className="aux-account-group-column"><Select value={account.billing_group ? `group:${account.billing_group}` : "independent"} onValueChange={(value) => updateAccount(setDraftAccounts, account.account_id, { billing_group: value === "independent" ? "" : value.slice(6) })}>
            ...
            <TableCell className="aux-account-actions-column"><Button type="button" size="sm" className="aux-account-save" disabled={savingAccount === account.account_id} onClick={() => void saveAccount(account)}>{savingAccount === account.account_id ? "保存中…" : "保存"}</Button></TableCell>
          </TableRow>)}
        </TableBody></Table></div>
